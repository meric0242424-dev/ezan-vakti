...
class PrayerTimesRepository(private val prefsManager: PrefsManager) {
    ...
    suspend fun getForOffset(location: LocationInfo, dayOffset: Int): Result<PrayerDay> {
        ...
        return monthResult.mapCatching { days ->
            days.getOrNull(dayOfMonth - 1) ?: days.first()
        }
    }

    suspend fun getToday(location: LocationInfo): Result<PrayerDay> = getForOffset(location, 0)
